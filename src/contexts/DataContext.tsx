...
// ... (celá sekce s typy zůstává beze změny)
...
export function DataProvider({ children }: { children: ReactNode }) {
    ...
    // Stavy pro filtry zůstávají, uživatel do nich píše okamžitě
    ...
    // <-- 2. VYTVOŘÍME "ZPOŽDĚNÉ" HODNOTY
    const debouncedStartupSearch = useDebounce(startupSearch, 500); // 500ms prodleva
    ...
    const refetchStartups = useCallback(async (currentPage = 0) => {
        ...
        } else if (debouncedStartupSearch) { // <-- ZMĚNA ZDE
        ...
        setLoadingStartups(false);
    }, [debouncedStartupSearch, startupCategories, startupSortBy]); // <-- 4. PŘIDÁME DO ZÁVISLOSTÍ
    ...
    const refetchStudents = useCallback(async (currentPage = 0) => {
        ...
        } else if (debouncedStudentSearch) { // <-- ZMĚNA ZDE
        ...
        setLoadingStudents(false);
    }, [debouncedStudentSearch, studentSkills, studentSortBy]); // <-- 4. PŘIDÁME DO ZÁVISLOSTÍ
    
    useEffect(() => {
        setStartupPage(0);
        refetchStartups(0);
    }, [debouncedStartupSearch, startupCategories, startupSortBy, refetchStartups]); // <-- ZMĚNA ZDE

    useEffect(() => {
        setStudentPage(0);
        refetchStudents(0);
    }, [debouncedStudentSearch, studentSkills, studentSortBy, refetchStudents]); // <-- ZMĚNA ZDE
    ...
    const value = useMemo(() => ({
        ...
        loadingStartups, loadingStudents,
    ...
    }), [startups, students, allCategories, allSkills, loadingStartups, loadingStudents, hasMoreStartups, hasMoreStudents, startupSearch, startupCategories, startupSortBy, studentSearch, studentSkills, studentSortBy, loadMoreStartups, loadMoreStudents, refetchStartups, refetchStudents]);
