...
export default function SkillSelector({ onSelectionChange, initialSelectedIds = [], allSkills }: SkillSelectorProps) {
  ...
  return (
    <div className='flex flex-col items-center w-full'>
      ...
      <div className="w-full max-w-4xl px-4 sm:px-8 flex justify-between items-center mb-8">
            <h4 className="text-md sm:text-lg font-semibold opacity-70 text-[var(--barva-primarni)]">{getSkillsTitle()}</h4>
            {!searchTerm && (
                <button type="button" onClick={() => setShowAll(!showAll)} className="text-sm sm:text-md font-semibold text-[var(--barva-primarni)] flex items-center gap-1">
      ...
      <div className="w-full max-w-4xl px-4 sm:px-8 flex flex-wrap justify-center gap-3 md:gap-4">
        {displayedSkills.map(skill => (
          <button
            key={skill.id}
            type="button"
            onClick={() => handleToggleSkill(skill.id)}
            className={`px-4 py-1.5 sm:px-4 sm:py-2 text-[var(--barva-primarni)] rounded-full font-bold sm:font-light text-sm sm:text-xl outline-2 transition-colors duration-200 cursor-pointer
