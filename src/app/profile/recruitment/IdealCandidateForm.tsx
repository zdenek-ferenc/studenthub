...
export default function IdealCandidateForm() {
    ...
    const { register, control, handleSubmit, watch, reset, formState: { errors, isSubmitting, dirtyFields } } = useForm<FormData>({
    ...
    const hasExistingData = !!watchedData.ideal_candidate_description || watchedData.technologies.length > 0;
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            const [profileRes, techRes, allTechRes] = await Promise.all([
                supabase.from('StartupProfile').select('ideal_candidate_description').eq('user_id', user.id).single(),
                supabase.from('StartupTechnology').select('technology_id').eq('startup_id', user.id),
            ...
            if (profileRes.data && techRes.data) {
                reset({
                    ideal_candidate_description: profileRes.data.ideal_candidate_description || '',
    ...
    const onSubmit = async (formData: FormData) => {
        ...
        const [profileResult, techResult] = await Promise.all([
            supabase.from('StartupProfile').update({ ideal_candidate_description: formData.ideal_candidate_description }).eq('user_id', user.id),
            (async () => {
                await supabase.from('StartupTechnology').delete().eq('startup_id', user.id);
                const toInsert = formData.technologies.map(techId => ({ startup_id: user.id, technology_id: techId }));
    ...
    const handleDelete = async () => {
        ...
        const [profileResult, techResult] = await Promise.all([
            supabase.from('StartupProfile').update({ ideal_candidate_description: null }).eq('user_id', user.id),
            supabase.from('StartupTechnology').delete().eq('startup_id', user.id)
        ]);
        if (profileResult.error || techResult.error) {
            showToast('Smazání se nezdařilo.', 'error');
        } else {
            reset({ ideal_candidate_description: '', technologies: [] });
            showToast('Sekce "Ideální kandidát" byla smazána.', 'success');
        }
    };
    
    const hasPreviewData = !!watchedData.ideal_candidate_description && technologiesForPreview.length > 0;
