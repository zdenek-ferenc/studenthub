  ...
  return (
    <div className='w-full space-y-6'>
      <div>
        ...
        <div className="p-3 bg-gray-100 rounded-lg min-h-[4rem] flex flex-wrap gap-2">
          {selectedCategoriesObjects.length > 0 ? (
            selectedCategoriesObjects.map(cat => (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 bg-[var(--barva-primarni2)] text-[var(--barva-primarni)] rounded-full text-sm font-semibold hover:opacity-80 transition-all ease-in-out duration-200 cursor-pointer"
              >
                {cat.name}
                <X size={14} />
              </button>
      ...
      <div>
        ...
        />
        <div className="p-3 bg-gray-100 rounded-lg max-h-60 overflow-y-auto flex flex-wrap gap-2">
          {availableCategories.map(cat => (
            <button
              key={cat.id}
              type="button"
              onClick={() => handleToggleCategory(cat.id)}
              className="cursor-pointer px-3 py-1.5 bg-white text-gray-700 rounded-full text-sm hover:bg-[var(--barva-svetle-pozadi)] hover:border-[var(--barva-primarni)] hover:text-[var(--barva-primarni)] transition-all ease-in-out duration-200"
            >
              + {cat.name}
            </button>
          ))}
