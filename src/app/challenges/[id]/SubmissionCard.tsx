...
import { User, Edit, Clock, EyeOff, Lock } from 'lucide-react';
...
export default function SubmissionCard({
  ...
  const handleUnlock = async () => {
    if (!challengeId || !prizeAmount) return;
  ...
  return (
    <div className="relative bg-white p-6 rounded-2xl shadow-xs border border-gray-100 flex flex-col h-full gap-4">
      ...
      {isLocked && (
        <div className="absolute inset-0 backdrop-blur-md bg-white/30 border border-white/20 shadow-xl rounded-2xl z-20 flex flex-col items-center justify-center p-6 text-center">
          <Lock className="w-12 h-12 text-[var(--barva-primarni)] mb-4 opacity-80" />
          <h3 className="text-xl font-bold text-[var(--barva-tmava)] mb-2">Řešení je uzamčeno</h3>
          <p className="text-gray-600 mb-6 text-sm">Pro zobrazení řešení a výběr vítěze je nutné složit odměnu.</p>
          <button
            onClick={handleUnlock}
            className="px-6 py-3 rounded-full bg-[var(--barva-primarni)] text-white font-bold shadow-lg hover:bg-[var(--barva-primarni)]/90 transition-all transform hover:scale-105"
          >
            Složit odměnu ({prizeAmount ? (prizeAmount / 100).toLocaleString('cs-CZ') : 0} Kč)
          </button>
        </div>
      )}
      ...
      <div>
        {submission.status === 'submitted' || isReviewed ? (
          <a href={isLocked ? '#' : (submission.file_url || '#')} target={isLocked ? undefined : "_blank"} rel="noopener noreferrer" className={`w-full text-center block px-6 py-2 rounded-full font-semibold transition-all ease-in-out duration-200 ${isLocked ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-[var(--barva-primarni)] text-white hover:bg-[var(--barva-primarni)]/90'}`}>
            Stáhnout řešení
          </a>
        ) : (
