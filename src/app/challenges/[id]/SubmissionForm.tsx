...
// Odebrali jsme nepoužívaný import 'useRouter'
...
export default function SubmissionForm({ challengeId, submissionId, initialSubmission, onSuccess }: SubmissionFormProps) {
  ...
  // Stav, který určuje, jestli už bylo řešení odevzdáno
  const isSubmitted = initialSubmission?.status === 'submitted';

  // Tento efekt se postará o zobrazení již nahraného souboru
  useEffect(() => {
  ...
  const onSubmit = async (data: SubmissionFormData) => {
    ...
    if (error) {
      alert("Odevzdání se nezdařilo.");
    } else {
      alert("Řešení bylo úspěšně odevzdáno!");
      onSuccess(); // Zavoláme "instrukci" od rodiče, aby obnovil data
    }
  ...
  return (
    <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100 mt-8">
      <h2 className="text-2xl font-bold text-center text-[var(--barva-tmava)] mb-6">
        {isSubmitted ? 'Odevzdané řešení' : 'Řešení'}
      </h2>
          ...
          {!isSubmitted && !uploadedFile && (
            <>
              ...
              <label 
                ...
                className="w-full flex items-center gap-3 p-3 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-[var(--barva-primarni)] hover:bg-blue-50 transition-colors"
              >
                <span className="flex items-center justify-center w-6 h-6 rounded-full border-2 border-[var(--barva-primarni)] text-[var(--barva-primarni)] font-bold text-lg">
                  +
                </span>
                <span className="font-semibold text-[var(--barva-primarni)]">
                  {isUploading ? 'Nahrávám...' : 'Připojit přílohu'}
                </span>
              </label>
