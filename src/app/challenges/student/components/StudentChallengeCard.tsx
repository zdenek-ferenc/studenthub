    ...
    <div className="relative bg-white rounded-2xl shadow-xs hover:shadow-none transition-all duration-300 ease-in-out border-2 border-gray-100 flex flex-col h-full pr-5 sm:pr-6">
      {isApplied ? (
        ...
        <button
          ...
          disabled={isSaving}
          className={`absolute top-3 right-3 z-10 p-1.5 cursor-pointer rounded-full transition-colors duration-200 ${isSaving
              ? 'text-gray-300'
              : isSaved
          ...
          <div className="flex flex-wrap px-4 md:px-5 py-2 gap-1 sm:gap-2">
            {(
              ...
              return (
                <span
                  key={Skill.id}
                  className={`flex items-center justify-center sm:gap-1.5 bg-[var(--barva-svetle-pozadi)] leading-none text-[var(--barva-primarni)] border border-[var(--barva-primarni)] px-2 py-1.5 3xl:px-3 3xl:py-2 rounded-full text-xs 3xl:text-sm font-semibold transition-colors ${isMatch
