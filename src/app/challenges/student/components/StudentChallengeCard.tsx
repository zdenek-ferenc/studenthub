...
const RewardsDisplay = ({ challenge }: { challenge: Challenge }) => {
    ...
    if (rewards.length === 0) {
      return <span className="font-semibold text-lg">Nefinanční</span>;
    }
  
    const topReward = Math.max(...rewards);
    return <span className="text-lg"><span className='text-[var(--barva-primarni)] font-bold'>{topReward}</span> Kč</span>
};
      ...
      <div className="border-t border-gray-100 mt-4 pt-4 p-6 flex items-end justify-between">
        <div className="grid grid-cols-2 gap-x-2 gap-y-1 lg:gap-x-4 lg:gap-y-2 text-sm text-gray-800 font-medium">
            <div className="flex items-center gap-2">
                ...
                <span><span className='text-[var(--barva-primarni)] font-bold'>{applicantCount}</span> / {challenge.max_applicants || '∞'}</span>
            </div>
            <div className="flex items-center gap-2">
        ...
        <Link href={`/challenges/${challenge.id}`} className="absolute right-3 bottom-3 leading-none">
          <div className="hidden xl:flex items-center justify-center px-5 py-3 rounded-full bg-[var(--barva-primarni)] text-white font-semibold hover:opacity-90 transition-opacity">
            Detail výzvy
          </div>
